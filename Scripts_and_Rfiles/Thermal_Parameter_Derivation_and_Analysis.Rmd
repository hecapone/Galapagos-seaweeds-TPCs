...
View(params)
write.csv(params, "../Macro_phys_TP/Output_analysis/combined_TPC_term_estimates.csv")
```
...
#QQ Plots by species, treatment type, and rate type
```{r}
library(ggplot2)
...
##Do this for every treatment combination##
```
...
#######Thermal parameter mean (+/-95% CI) bar plots for NP & R####### 

#Net photosynthesis param mean plots
...
#Arrange the NP plots into a 2x2 panel grid
```{r}
library(gridExtra)
grid.arrange(Topt_NP, Pmax_NP, E_NP, Eh_NP)
```


#Respiration param mean plots

#Pmax_R
```{r}
pmax_data_R<-read.csv('pmax.csv')

# Define the custom order of Species
species_order <- c("Grac", "Pad", "Caul", "Ulv")

# Convert Species to factor with the custom order
pmax_data_R$Species <- factor(pmax_data_R$Species, levels = species_order)

Pmax_data_filtered <- pmax_data_R %>%
  filter(rate.type == "Respiration" & Ambient_Nutrient %in% c("Ambient"))
...
Pmax_R<- ggplot() + 
  ...
  theme(axis.text.x = element_text(face = "italic", size = 11, color = "black"),
        ...
        plot.title = element_text(size = 27, face = "bold", hjust = -.1)) +
  scale_x_discrete(labels = c("Ochtodes", "Padina", "Caulerpa", "Ulva")) +
  scale_y_continuous(limits = c(3, 5.5), breaks = seq(3, 5, by = 1)) +
...
#Topt_R
```{r}
Topt_data_R<-read.csv('Output_analysis/Topt.csv')
Topt_data_R$Topt <- Topt_data_R$Topt - 273.15

# Define the custom order of Species
species_order <- c("Grac", "Pad", "Caul", "Ulv")

# Convert Species to factor with the custom order
Topt_data_R$Species <- factor(Topt_data_R$Species, levels = species_order)

# Filter the data
Topt_data_filtered <- Topt_data_R %>%
  filter(rate.type == "Respiration" & Ambient_Nutrient %in% c("Ambient"))
...
Topt_R <- ggplot() + 
  ...
  theme(axis.text.x = element_text(face = "italic", size = 11, color = "black"),
        ...
        plot.title = element_text(size = 27, face = "bold", hjust = -.1)) +
  scale_x_discrete(labels = c("Ochtodes", "Padina", "Caulerpa", "Ulva")) +
  scale_y_continuous(limits = c(25, 43), breaks = seq(25, 43, by = 3))+
  theme(legend.text = element_text(size = 17),
            legend.title = element_text(size = 19),
                axis.title.x = element_text(size = 20),
...
#E_R
```{r}
E_data_R<-read.csv('Output_analysis/E.csv')
#E_data_NP$E <- E_data_NP$E - 273.15
...
# Convert Species to factor with the custom order
E_data_R$Species <- factor(E_data_R$Species, levels = species_order)

# Filter the data
E_data_filtered <- E_data_R %>%
  filter(rate.type == "Respiration" & Ambient_Nutrient %in% c("Ambient"))
...
E_R <- ggplot() + 
  ...
  theme(axis.text.x = element_text(face = "italic", size = 11, color = "black"),
        ...
        plot.title = element_text(size = 27, face = "bold", hjust = -.1)) +
  scale_x_discrete(labels = c("Ochtodes", "Padina", "Caulerpa", "Ulva")) +
  scale_y_continuous(limits = c(0, 1.48), breaks = seq(0, 1.5, by = 0.5))+
  theme(legend.text = element_text(size = 17),
              legend.title = element_text(size = 19),
        ...
        axis.text.y = element_text(size = 18),
       plot.title = element_text(size = 27, face = "bold")) +
ggtitle(expression(bold("c  ")~"Log "~italic(E)))+
#ggtitle(expression(paste("Log ", italic("E")))) +
labs(x = "Genera", y = "Energy (eV)") +
scale_color_manual(values = c("deepskyblue1")) 
...
#Eh_R
```{r}
Eh_data_R<-read.csv('Output_analysis/Eh.csv')
#E_data_NP$E <- E_data_NP$E - 273.15
...
# Convert Species to factor with the custom order
Eh_data_R$Species <- factor(Eh_data_R$Species, levels = species_order)

# Filter the data
Eh_data_filtered <- Eh_data_R %>%
  filter(rate.type == "Respiration" & Ambient_Nutrient %in% c("Ambient"))
...
Eh_R <- ggplot() + 
  ...
  theme(legend.text = element_text(size = 17),
             legend.title = element_text(size = 19),
                ...
                axis.text.y = element_text(size = 18),
       plot.title = element_text(size = 27, face = "bold")) +
ggtitle(expression(bold("d  ")~"Log "~italic(E[plain(h)]))) +
labs(x = "Genera", y = "Energy (eV)") +
scale_color_manual(values = c("deepskyblue1")) 
...
#Arrange the R plots into a 2x2 panel grid
```{r}
library(gridExtra)
grid.arrange(Topt_R, Pmax_R, E_R, Eh_R)
```
...
# 
# # Create the ggplot with the filtered data
# Grac_all <- ggplot(data = mydatagrac) +
#   geom_point(aes(K - 273.15, log.rate, col = Condition), size = 2) +
...
# 
# # Create the ggplot with the filtered data
# Caul_all <- ggplot(data = mydatacaul) +
#   geom_point(aes(K - 273.15, log.rate, col = Condition), size = 2) +
...
#Arrange the mean plots into a 2x2 panel grid
```{r}
...
#Arrange the all individuals plots into a 2x2 panel grid
```{r}
#install.packages("gridExtra")
# library(gridExtra)
# 
# grid_all <- grid.arrange(Grac_all, Pad_all, Caul_all, Ulv_all, ncol = 2)
# 
# grid_all
```
