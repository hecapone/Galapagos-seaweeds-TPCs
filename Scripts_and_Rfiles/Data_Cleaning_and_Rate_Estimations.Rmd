---
title: "Data Cleaning and Rate Estimations"
...
#Calculating O2 evolution for every file in list
```{r}
...
#NP mean curve comparisons across all species by treatment condition
```{r}

###PLOTS COMPARING TREATMENT AND RATE TYPE ACROSS SPECIES

rawslope<- ggplot(Photo.RNP, aes(x=Temp.C, y=log.rate, group=Ambient_Nutrient, color=Ambient_Nutrient)) +
...
# Define the color palette and new species names
species_colors <- c("Caul" = "olivedrab", "Grac" = "orangered2", "Pad" = "lightsalmon4", "Ulv" = "olivedrab3")
species_names <- c("Caulerpa", "Ochtodes", "Padina", "Ulva")
...
# Update the plot
# Update the plot
NP <- ggplot(Photo.RNP, aes(x=Temp.C, y=log.rate, group=Species, color=Species)) +
      geom_point() +
      geom_smooth(aes(fill=Species), se=TRUE, span=1.5, alpha=0.5) +
      geom_smooth(se=TRUE, span=1.5, linetype="solid", alpha=0.25) +
      theme_classic() +
      facet_wrap(~gsub("Nutrient", "Enriched", Ambient_Nutrient)*rate.type,
                 labeller=labeller(.multi_line=FALSE),
                 scales="free_y") +
      labs(y = expression(Rate~(log~mu~mol~O[2]~cm^-2~hr^-1)), x="Temperature (ºC)") +
      scale_color_manual(values=species_colors, labels=species_names, drop = FALSE) +
      scale_fill_manual(values=species_colors, labels=species_names, drop = FALSE) +
      labs(color = "Genus", fill = "Genus") +
      theme(strip.text = element_text(size = 12),
            legend.title = element_text(size = 13),
            axis.title.x = element_text(size = 15),
            axis.title.y = element_text(size = 15)) +
      guides(color = FALSE,
             fill = guide_legend(override.aes = list(color = species_colors)))

# Increase legend size
NP + theme(legend.text = element_text(size=13),
...
#NP & R regressions plotted by treatment per species
...
##Plots for R and NP across all species
```{r}
library(gridExtra)
library(ggplot2)
