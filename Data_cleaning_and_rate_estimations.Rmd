...
attach(RPROJ)
path.p<-file.path(hcapone2, 'Macroalgae_respfiles_clean')
path.p
```
...
Sample.Info <- read.csv(file=paste0(path.p,"/../Macro_metadata.csv"), header=T) 
View(Sample.Info)
```
...
```{r}
####Ulva####
#MERGE NP AND R REGRESSIONS INTO A SINGLE PLOT BY SPECIES BASED ON RATE AND TREATMENT.TYPE
library(ggplot2)
...
library(gridExtra)
library(ggplot2)
##Plots for R and NP across species
